...
public class ValidateStructureValue extends ModelEventDelegate<X_GH_Structure>{
	...
	public void validateValue() {
		...
		if(record.getGH_ExpenseObject_ID() != v_representationExpenseCode) {
			count = DB.getSQLValue(record.get_TrxName(), "SELECT count(*) FROM GH_Structure WHERE C_Year_ID = ? AND Value = ?",record.getC_Year_ID(),record.getValue());	
			if(count>0)
		...
		//pero solo 1
			...
			count = DB.getSQLValue(record.get_TrxName(), "SELECT count(*) FROM GH_Structure WHERE C_Year_ID = ? AND Value = ? AND GH_ExpenseObject_ID = ?",record.getC_Year_ID(),record.getValue(),v_representationExpenseCode);
			if(count>0)
